...
<el-table-column label="使用期限" width="150" align="center">
    <template slot-scope="{row}">
        <small>{{ row.start_time | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }} 至 {{ row.end_time | parseTime('{y}-{m}-{d} {h}:{i}:{s}') }}</small>
    </template>
</el-table-column>
...
<el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
    <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="90px" style="width: 700px; margin-left:50px;">
        ...
        <el-form-item label="面值" prop="price">
            <el-input-number v-model="temp.price" size="mini" :min="0" :step="0.1" :controls="true" controls-position="both" :precision="2" >
            </el-input-number>元
        </el-form-item>
